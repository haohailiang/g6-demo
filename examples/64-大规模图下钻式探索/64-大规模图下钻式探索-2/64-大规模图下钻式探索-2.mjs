import graphData from "./data.mjs";
import registerAggregatedNode from "./node/aggregated-node.mjs";
import registerRealNode from "./node/real-node.mjs";
import registerCustomQuadraticEdge from "./edge/custom-quadratic.mjs";
import registerCustomLineEdge from "./edge/custom-line.mjs";

const { labelPropagation, louvain, findShortestPath } = G6.Algorithm;
const { uniqueId } = G6.Util; // 获取独一无二的ID函数

const NODE_SIZE_MAPPING = "degree";
const SMALL_GRAPH_LABEL_MAX_LENGTH = 5;
let labelMaxLength = SMALL_GRAPH_LABEL_MAX_LENGTH;
const DEFAULT_NODE_SIZE = 20;
const DEFAUL_TAG_GREGATED_NODE_SIZE = 53; // 默认聚合大小53个
...
registerRealNode(global);
registerCustomQuadraticEdge(animateOpacity);
registerCustomLineEdge(animateOpacity, duration, virtualEdgeOpacity);
...
const processNodesEdges = (
    ...
    nodes.forEach(node => {
        ...
        if (cachePosition) {
            ...
            if (manipulatePosition && !node.x && !node.y) {
                node.x =
                    manipulatePosition.x +
                    30 * Math.cos(Math.random() * Math.PI * 2);
                node.y =
                    manipulatePosition.y +
    ...
    nodes.sort(descendCompare(NODE_SIZE_MAPPING));
    ...
    nodes.forEach((node, i) => {
        ...
        node.size = isRealNode ? DEFAULT_NODE_SIZE : DEFAUL_TAG_GREGATED_NODE_SIZE;
    ...
    edges.forEach(edge => {
        ...
        if (sourceNode.x && !targetNode.x) {
            targetNode.x =
                sourceNode.x +
                sourceDis * Math.cos(Math.random() * Math.PI * 2);
        }
        if (sourceNode.y && !targetNode.y) {
            targetNode.y =
                sourceNode.y +
                sourceDis * Math.sin(Math.random() * Math.PI * 2);
        }
        if (targetNode.x && !sourceNode.x) {
            sourceNode.x =
                targetNode.x +
                targetDis * Math.cos(Math.random() * Math.PI * 2);
        }
        if (targetNode.y && !sourceNode.y) {
            sourceNode.y =
                targetNode.y +
                targetDis * Math.sin(Math.random() * Math.PI * 2);
        }
    ...
    descreteNodes.forEach(node => {
        if (!node.x && !node.y) {
            node.x =
                descreteNodeCenter.x +
                30 * Math.cos(Math.random() * Math.PI * 2);
            node.y =
                descreteNodeCenter.y +
                30 * Math.sin(Math.random() * Math.PI * 2);
        }
    });
...
function getData(data) {
            ...
            switch (liIdStrs[0]) {
                ...
                case "collapse":
                    const aggregatedNode =
                        aggregatedNodeMap[model.clusterId];
                    manipulatePosition = {
                        x: aggregatedNode.x,
                        y: aggregatedNode.y
                    };
    ...
    // 渲染残影问题 https://g6.antv.antgroup.com/manual/faq/faq#rendering-residuals
